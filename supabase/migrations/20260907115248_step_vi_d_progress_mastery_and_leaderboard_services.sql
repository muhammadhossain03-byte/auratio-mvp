-- Auratio Step VI-D — private mastery, Approved history, and deterministic public leaderboard services.

create type public.leaderboard_period as enum ('all_time', 'monthly');

create or replace function private.alr_activity_decay(
  p_latest_approved_at timestamptz,
...
declare
  v_days integer;
begin
  if p_latest_approved_at is null then
    return null;
  end if;

  v_days := greatest(
    0,
    floor(extract(epoch from (p_as_of - p_latest_approved_at)) / 86400.0)::integer
  );

  if v_days <= 30 then return 1.00;
  elsif v_days <= 44 then return 0.95;
  elsif v_days <= 58 then return 0.90;
  elsif v_days <= 72 then return 0.85;
  else return 0.80;
  end if;
end;
$$;
revoke execute on function private.alr_activity_decay(timestamptz,timestamptz) from public, anon, authenticated;
...
begin
  ...
  with scoped as (
    ...
    select
      r.*,
      case when participation_count>=3 then
        rank() over(
  ...
  with scoped as (
    ...
    select r.*,case when participation_count>=3 then rank() over(order by alr desc,full_window_average desc,best_single_score desc,qualification_at asc) end leaderboard_rank
    from rated r
  )
  ...
  left join ranked r on r.user_id=a.actor_id;
