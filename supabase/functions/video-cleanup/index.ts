...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-auratio-cron-token",
...
type BackendCredentials = {
  ...
  acceptedServiceKeys: Set<string>;
};

function readBackendCredentials(): BackendCredentials {
  const acceptedServiceKeys = new Set<string>();
  ...
  if (rawSecretKeys) {
    try {
      ...
      if (typeof defaultKey === "string" && defaultKey.trim()) {
        serviceKey = defaultKey.trim();
        acceptedServiceKeys.add(serviceKey);
      }
    } catch {
  ...
  if (legacyServiceRoleKey) {
    acceptedServiceKeys.add(legacyServiceRoleKey);
    if (!serviceKey) {
      serviceKey = legacyServiceRoleKey;
    }
  }
...
async function cronRequestAuthorized(
  service: ReturnType<typeof createClient>,
  req: Request,
): Promise<boolean> {
  const token =
    (req.headers.get("x-auratio-cron-token") ?? "").trim();
  ...
  if (error) {
    console.error("video_cleanup_cron_auth_failed", {
...
Deno.serve(async (req: Request) => {
  ...
  const credentials = readBackendCredentials();

  if (!supabaseUrl || !credentials.serviceKey) {
  ...
  const internalAuthorized =
    callerApiKey.length > 0 &&
    credentials.acceptedServiceKeys.has(callerApiKey);

  const cronAuthorized = internalAuthorized
    ? false
    : await cronRequestAuthorized(service, req);
  ...
  for (const job of claimed) {
    ...
    const submissionId =
      typeof job.submission_id === "string"
        ? job.submission_id
        : "";

    const bucketName =
      typeof job.bucket_name === "string"
        ? job.bucket_name
        : "";
