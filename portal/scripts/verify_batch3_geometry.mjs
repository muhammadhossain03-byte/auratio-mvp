...
async function inspectBatch3Geometry() {
  ...
  try {
    ...
    const routes = [
      '/volunteer/evaluation/sub-8821/criterion',
      '/volunteer/evaluation/sub-8821/review',
      '/volunteer/evaluation/sub-8821/submitted',
      '/volunteer/completed',
      '/volunteer/completed/sub-8821',
      '/volunteer/completed/sub-8792',
      '/volunteer/completed/sub-8755',
      '/volunteer/completed/sub-8741',
      '/volunteer/evaluation/sub-8821/reopened',
    ]
    ...
    for (const route of routes) {
      ...
      if (!geo.sidebar || geo.sidebar.width !== 230 || geo.sidebar.height !== 900) {
        throw new Error(`Sidebar geometry invalid on ${route}`)
      }
      if (!geo.topbar || geo.topbar.width !== 1136 || geo.topbar.height !== 72) {
        throw new Error(`Topbar geometry invalid on ${route}`)
      }
      if (!geo.title || geo.title.x !== 260 || (geo.title.y !== 106 && geo.title.y !== 104)) {
        throw new Error(`Title geometry invalid on ${route}: expected x=260, y=104 or 106, got x=${geo.title?.x}, y=${geo.title?.y}`)
      }
      if (!geo.subtitle || geo.subtitle.x !== 260) {
        throw new Error(`Subtitle geometry invalid on ${route}`)
      }
    ...
    console.log('\nALL 9 BATCH 3 ROUTES PASSED GEOMETRY VALIDATION!')
...
inspectBatch3Geometry().catch((err) => {
  console.error('Geometry verification failed:', err)
  process.exit(1)
})
