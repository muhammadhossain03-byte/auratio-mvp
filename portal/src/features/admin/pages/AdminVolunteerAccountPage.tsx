...
import { PersistedAdminVolunteerAccountPage } from "./PersistedAdminVolunteerAccountPage";
import {
...
function PrototypeAdminVolunteerAccountPage() {
  ...
  const volunteerTracks =
    getVolunteerTrackEligibility(resolvedId) ?? volunteer.selectedTracks ?? [];
  ...
  const effectiveAvailability = availabilityState
    ...
    : volunteer.effectiveAvailability;
  const declaredAvailability = availabilityState
    ? availabilityState.declaredAvailability
    : isDeactivated || isInvited
      ? "—"
      : "Available";
  const overrideReason = availabilityState
    ? availabilityState.overrideReason
    : "None";
  const tracksDisplay =
    volunteerTracks.length > 0
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Track eligibility, effective availability, and active-assignment count
          are separate fields. The count provides workload context without
          creating an automatic assignment prohibition.
        </div>
      </div>
    </AdminLayout>
  );
...
export function AdminVolunteerAccountPage() {
  return portalSupabaseRuntimeMode() === "configured" ? (
    <PersistedAdminVolunteerAccountPage />
  ) : (
    <PrototypeAdminVolunteerAccountPage />
  );
}
