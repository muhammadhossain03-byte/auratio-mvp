...
function PrototypeAdminAvailabilityOverridePage() {
  ...
  const currentEffective =
    availabilityState?.effectiveAvailability ?? volunteer.effectiveAvailability;
  const currentDeclared =
    availabilityState?.declaredAvailability ??
    (volunteer.lifecycle === "Active" ? "Available" : "—");

  const targetOverrideStatus =
    currentEffective === "Available" ? "Unavailable" : "Available";
      ...
      >
        ...
        <div style={{ marginTop: "20px" }}>
          ...
          >
            ...
            <span
              style={{ fontSize: "11px", color: "#6B788A", marginLeft: "12px" }}
            >
...
export function AdminAvailabilityOverridePage() {
  ...
  if (portalSupabaseRuntimeMode() === "configured") {
    // Configured runtime is read-only for Volunteer availability.
    return (
      <Navigate
        to={
          volunteerId
            ? `/admin/volunteers/${volunteerId}`
            : portalRoutePaths.admin.volunteers
        }
        replace
      />
    );
  }
  return <PrototypeAdminAvailabilityOverridePage />;
}
