...
export const HUMAN_FIX_H12_DIR = path.resolve('capture_output', 'human_fix_h12')
...
export async function captureHumanFixH12Screenshot(page: Page, filename: string): Promise<string> {
  if (!fs.existsSync(HUMAN_FIX_H12_DIR)) {
    fs.mkdirSync(HUMAN_FIX_H12_DIR, { recursive: true })
  }
  const filePath = path.join(HUMAN_FIX_H12_DIR, filename)
  await page.screenshot({ path: filePath, fullPage: false })
  return filePath
}
